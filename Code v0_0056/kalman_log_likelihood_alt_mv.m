function [ L ] = kalman_log_likelihood_alt_mv(model_info, parameters)
...
% try
    ...
    %      y[t]   = Zx[t] + H eps[t]
    %
    Z=zeros(ny,nx+ne*(q+max(p,1)));
    Z(:,1:nx)=parameters.X.Upsilon;%This matrix selects the observables as a linear combo of enodgenous variables
    A=zeros(nx+ne*(q+max(p,1)),nx+ne*(q+max(p,1)));
    A(1:nx,1:nx)=parameters.X.Lambda;
    R=zeros(nx+ne*(q+max(p,1)),ne);
    R(1:nx,1:ne)=parameters.X.Phi(:,1:ne);
    R(nx+1:nx+ne,1:ne)=eye(ne);
    ...
    xhat_tt  = zeros(nx+ne*(q+max(p,1)),1);
    Sigma_tt = reshape( (eye((nx+ne*(q+max(p,1)))^2)-kron(A,A) ) \ Q(:), nx+ne*(q+max(p,1)),nx+ne*(q+max(p,1)) );
    logsum = 0;
    sumsq  = 0;
